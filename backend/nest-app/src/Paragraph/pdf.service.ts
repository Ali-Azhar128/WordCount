  ...
  constructor(private readonly paraDOW: ParaDOW) {}
    async generatePDF(paraId: string, paragraph: string, count: number): Promise<string>{
        try {
            ...
            const templatePath = path.join(__dirname, '../../src/Paragraph/Template/pdfTemplate.ejs');
            const content = await ejs.renderFile(templatePath, { paragraph, wordCount: count });
